...
public class EventIpcManagerDefaultImpl implements EventIpcManager, EventIpcBroadcaster, InitializingBean {
    ...
    private Integer m_handlerPoolSize = 300;
    
    private Integer m_handlerQueueLength = 100000;
    ...
    public void sendNowSync(Log eventLog) {
        ...
        try {
			//Logging.withPrefix(Eventd.LOG4J_CATEGORY, m_eventHandler.createCallable(eventLog, true));
       // 	CompletableFuture<Log> futureLog = m_eventHandler.createCallable(eventLog, true);
		} catch (Exception e) {
    ...
    public CompletableFuture<Log> getEnrichedEvent(Log eventLog) throws InterruptedException, ExecutionException {
    		Objects.requireNonNull(eventLog);
    		CompletableFuture <Log> futureLog = m_eventHandler.getEnrichedEvent(eventLog,true);
    		return futureLog;
    }
