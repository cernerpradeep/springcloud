...
public class NodeDaoHibernate extends AbstractDaoHibernate<OnmsNode, Integer> implements NodeDao {
    ...
    public List<OnmsNode> findAllByCategoryLists( final Collection<OnmsCategory> rowCategories, final Collection<OnmsCategory> columnCategories) {
        ...
        return hibernateTemplate.execute(new HibernateCallback<List<OnmsNode>>() {
    ...
    public SurveillanceStatus findSurveillanceStatusByCategoryLists(final Collection<OnmsCategory> rowCategories, final Collection<OnmsCategory> columnCategories) {
        return hibernateTemplate.execute(new HibernateCallback<SurveillanceStatus>() {
    ...
    public Map<String, Integer> getForeignIdToNodeIdMap(String foreignSource) {
        List<Object[]> pairs = (List<Object[]>)hibernateTemplate.find("select n.id, n.foreignId from OnmsNode n where n.foreignSource = ?", foreignSource);
    ...
    public void deleteObsoleteInterfaces(Integer nodeId, Date scanStamp) {
        hibernateTemplate.bulkUpdate("delete from OnmsIpInterface ipInterface where ipInterface.node.id = ? and ipInterface.isSnmpPrimary != 'P' and (ipInterface.ipLastCapsdPoll is null or ipInterface.ipLastCapsdPoll < ?)", new Object[] { nodeId, scanStamp });
        hibernateTemplate.bulkUpdate("delete from OnmsSnmpInterface snmpInterface where snmpInterface.node.id = ? and (snmpInterface.lastCapsdPoll is null or snmpInterface.lastCapsdPoll < ?)", new Object[] { nodeId, scanStamp });
    }
    ...
    public Map<String, Long> getNumberOfNodesBySysOid() {
        List<Object[]> pairs = (List<Object[]>)hibernateTemplate.find("select n.sysObjectId, count(*) from OnmsNode as n where n.sysObjectId != null group by sysObjectId");
